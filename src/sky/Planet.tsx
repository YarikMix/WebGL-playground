...
void main() {
  ...
  /* Показатель светлой кромки — 7, а не 18. Замер: у pow(rim, 18) полуспад приходится на
     n.z = 0.038, то есть на 1 пиксель внутрь от силуэта (R ≈ 1368 px, d ≈ R·n.z²/2) — деталь
     тоньше пикселя. Из-за этого она не ехала вместе с планетой, а скакала через порог
     растеризации, и буфер преломления стекла (мип-уровни + lod-смещение drei ~0.9 при
     roughness 0.1/ior 1.4) превращал скачок в белую вспышку 2–5 px в случайном месте дуги.
     Отсюда «мерцающие белые точки» по низу карточки: снаружи стекла их не было ни одной,
     внутри — 0.5…1.2 за кадр. Гашение этого слагаемого давало ровно 0 вспышек за 6 кадров при
     средней яркости дуги 131.7 против 133.0 — то есть весь вклад волоска в картинку 1.3 единицы.
     При показателе 7 полуспад уходит на ~6 px: кромка остаётся тонкой светлой линией, но её
     уже есть чем разрешить, и вспышки прекращаются. Амплитуда снижена 0.8 → 0.5, чтобы
     суммарная светимость линии выросла не в шесть раз, а примерно вчетверо. */
  col += (VIOLET * 0.55 * pow(rim, 5.0) + vec3(0.9, 0.85, 1.0) * 0.5 * pow(rim, 7.0)) * sunSide;
